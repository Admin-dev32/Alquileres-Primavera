...
try {
    ...
    if (!$settings) {
        $insert = $pdo->prepare("INSERT INTO settings (business_name, created_at) VALUES ('Alquileres Primavera', NOW())");
        $insert->execute();
        $stmt = $pdo->query('SELECT * FROM settings LIMIT 1');
        $settings = $stmt->fetch();
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$alertaError) {
            $update = $pdo->prepare('UPDATE settings SET business_name = :business_name, business_address = :business_address, business_phone = :business_phone, business_whatsapp = :business_whatsapp, business_email = :business_email, default_notes = :default_notes, logo_path = :logo_path WHERE id = :id');
            $update->execute([
...
<p class="text-muted">Actualiza los datos principales del negocio.</p>
